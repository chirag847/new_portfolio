...
const ModernChatbot: React.FC<ChatbotProps> = ({ className }) => {
  ...
  const playTTS = useCallback(async (text: string, messageId: string) => {
    if (playingTTS === messageId) return; // Already playing this message
    
    setPlayingTTS(messageId);
    
    try {
      const audioBase64 = await sarvamService.textToSpeech(text);
      if (audioBase64) {
        await sarvamService.playAudio(audioBase64);
      }
    } catch (error) {
                              ...
                              <div className={`flex items-start gap-2 max-w-[80%] ${message.role === 'user' ? 'flex-row-reverse' : ''}`}>
                                ...
                                >
                                  ...
                                  <div className="flex items-center justify-between mt-2">
                                    ...
                                    {message.role === 'assistant' && sarvamService.isVoiceEnabled() && (
                                      <Button
                                        variant="ghost"
                                        size="sm"
                                        ...
                                        title="Play voice"
                                      >
                                        <Volume2 className={`h-3 w-3 ${playingTTS === message.id ? 'text-purple-600 animate-pulse' : 'text-gray-500'}`} />
                                      </Button>
                                    )}
                                  </div>
                                </motion.div>
